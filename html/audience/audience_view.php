        ...
        <div class="sub-contain">
		  ...
		  <button type="submit" class="btn-search" onclick='search_click("search")'>검색</button>
              ...
              <div class="information-box" id="information-box">
                <?php
                  $sql1 = mysqli_num_rows(query("SELECT * FROM $table WHERE end_date IS NULL AND participation = 0;"));
                  $sql2 = mysqli_num_rows(query("SELECT * FROM $table WHERE start_date IS NULL AND participation = 0;"));
                ?>
  	            ...
  	            <thead>
                    ...
                    <tbody>
                      <?php
                        if($val != "") {
                          if($key == "number") {
                            $result = query("SELECT * FROM $table WHERE end_date IS NULL AND participation = 0 AND number LIKE '%$val%' ORDER BY start_date");
                          }
                          else if($key == "name") {
                            $result = query("SELECT * FROM $table WHERE end_date IS NULL AND participation = 0 AND name LIKE '%$val%' ORDER BY start_date");
                          }
                        }
                        else {
                          $result = query("SELECT * FROM $table WHERE end_date IS NULL AND participation = 0 ORDER BY start_date");
                        }

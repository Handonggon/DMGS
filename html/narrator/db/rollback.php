<?php
  ...
  $firstSatDate =1 + ((6 - $firstday) % 7);
  $firstSunDate = 1 + ((7 - $firstday) % 7);
  ...
  for($i = $firstSatDate ; $i <= 31 ; $i = $i + 7) {//매주 월요일은 false로 저장(휴무)
    $result = query("UPDATE $table SET first_time = 0, second_time = 0 WHERE date = '$i'");
  }
  for($i = $firstSunDate ; $i <= 31 ; $i = $i + 7) {//매주 월요일은 false로 저장(휴무)
    $result = query("UPDATE $table SET first_time = 0, second_time = 0 WHERE date = '$i'");
  }
